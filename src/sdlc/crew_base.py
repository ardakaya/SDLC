...
def CrewBase(cls: T) -> T:
    ...
    class WrappedClass(cls):  # type: ignore
        print("DEBUG: THIS IS MY CrewBase.PY BEING LOADED")
        ...
        def load_configurations(self):
            ...
            if isinstance(self.original_agents_config_path, str):
                ...
                try:
                    # 🚀 FIXED → load only the "agents" part of the YAML
                    self.agents_config = self.load_yaml(agents_config_path).get('agents', {})
            ...
            if isinstance(self.original_tasks_config_path, str):
                ...
                try:
                    self.tasks_config = self.load_yaml(tasks_config_path).get('tasks', {})
                    print(f"DEBUG: tasks_config keys after load = {list(self.tasks_config.keys())}")  # 🚀 ADD HERE
                except FileNotFoundError:
                    logging.warning(
        ...
        def load_yaml(config_path: Path):
            print("DEBUG: ENTERING load_yaml()")  # 🚀 ADD THIS
            try:
                with open(config_path, "r", encoding="utf-8") as file:
                    data = yaml.safe_load(file)
                    print(f"DEBUG: raw yaml loaded from {config_path}: {data}")  # 🚀 ADD THIS
                    return data
            except FileNotFoundError:
